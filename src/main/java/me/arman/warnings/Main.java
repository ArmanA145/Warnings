...
public class Main extends JavaPlugin {
	...
	private void registerCommands() {
		...
		if (!(getConfig().getBoolean("categories.use"))) {
			...
			getCommand("Warnings").setExecutor(
					new WarningsCommandCategories(this));
		}
		if (!(getConfig().getBoolean("categories.use"))) {
			getCommand("ClearWarnings").setExecutor(
					new ClearWarningsCommand(this));
		} else {
			getCommand("ClearWarnings").setExecutor(
					new ClearWarningsCommandCategories(this));
		}
		if (!(getConfig().getBoolean("categories.use"))) {
			getCommand("SetWarnings").setExecutor(new SetWarningsCommand(this));
		} else {
			getCommand("SetWarnings").setExecutor(
					new SetWarningsCommandCategories(this));
		}
		if (!(getConfig().getBoolean("categories.use"))) {
			getCommand("AddWarnings").setExecutor(new AddWarningsCommand(this));
		} else {
			getCommand("AddWarnings").setExecutor(
					new AddWarningsCommandCategories(this));
		}
		if (!(getConfig().getBoolean("categories.use"))) {
			getCommand("TakeWarnings").setExecutor(
					new TakeWarningsCommand(this));
		} else {
			getCommand("TakeWarnings").setExecutor(
					new TakeWarningsCommandCategories(this));
		}

		...
		if (cmd.getName().equalsIgnoreCase("takewarnings")) {
			...
			if (args.length <= 2) {
				sender.sendMessage(ChatColor.RED
						+ "Usage: /takewarnings <player> <amount> <category>");
				return true;
				...
				if (current <= 0) {
					sender.sendMessage(ChatColor.RED + target.getName()
							+ " already has 0 warnings in category " + args[2]);
					return true;
				}
				int num = Integer.parseInt(args[1]);
				if (current - num < 0) {
					sender.sendMessage(ChatColor.RED + target.getName()
							+ " can't go below " + current
				...
				sender.sendMessage(ChatColor.GREEN + "Removed " + num
						+ " warnings from " + target.getName()
						+ " in category " + args[2]);
			}

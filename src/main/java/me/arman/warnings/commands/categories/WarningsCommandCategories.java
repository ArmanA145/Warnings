		...
		if (sender instanceof Player) {
			...
			if (!p.hasPermission("warnings.view")) {
				p.sendMessage(ChatColor.RED
						+ "You can not check your warnings!");
				return true;
			}
			if (args.length == 0) {
				ConfigurationSection info = plugin.dFile.getConfig()
				...
				if (!p.hasPermission("warnings.view.others")) {
					p.sendMessage(ChatColor.RED
							+ "You can not check other player's warnings!");
					return true;
				}
				ConfigurationSection info = plugin.dFile.getConfig()
						.getConfigurationSection(
								target.getUniqueId().toString() + ".warnings.");
				if (info == null) {
					p.sendMessage(ChatColor.GREEN + target.getName()
							+ " has no warnings!");
					return true;
				}
				p.sendMessage(ChatColor.RED

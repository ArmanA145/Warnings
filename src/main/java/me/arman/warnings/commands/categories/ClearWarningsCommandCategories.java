		...
		if (cmd.getName().equalsIgnoreCase("clearwarnings")) {
			if (!sender.hasPermission("warnings.clear")) {
				sender.sendMessage(ChatColor.RED
						+ "You can not clear warnings!");
				return true;
			}
			if (args.length == 0 || args.length == 1) {
				sender.sendMessage(ChatColor.RED
						+ "Usage: /clearwarnings <player> <category>");
				return true;
				...
				if (amount == 0) {
					sender.sendMessage(ChatColor.GREEN + target.getName()
							+ " has no warnings in category " + args[1]);
					return true;
				}
				...
				sender.sendMessage(ChatColor.GREEN + "Cleared "
						+ target.getName() + "\'s warnings in category "
						+ args[1]);
			}

		...
		if (cmd.getName().equalsIgnoreCase("warn")) {
			...
			if (args.length <= 1) {
				sender.sendMessage(ChatColor.RED
						+ "Usage: /warn <player> <category> <reason>");
				return true;
			}
			...
			if (args.length >= 3) {
				...
				int current = plugin.dFile.getConfig().getInt(
						target.getUniqueId().toString() + ".warnings."
								+ args[1], 0);
				List<String> commandList = plugin
						.getConfig()
						.getConfigurationSection(
								"categories." + args[1].replace(" ", "") + ".")
						.getStringList("punishments");
				if (current >= commandList.size()) {
					sender.sendMessage(ChatColor.RED
							+ "There are no more than " + commandList.size()
							+ " punishments configured for category \""
							+ args[1] + "\"!");
					return true;
				}
					...
					for (Player all : Bukkit.getServer().getOnlinePlayers()) {
						if (all.hasPermission("warnings.receivebroadcast")) {
							all.sendMessage(ChatColor
									.translateAlternateColorCodes('&', bmsg)
									.replace("%warner%", sender.getName())
									.replace("%player%", target.getName())
									.replace("%reason%", warnmessage)
				...
				if (plugin.getConfig().getBoolean("logSpecificInfo")) {
					List<String> l = plugin.dFile.getConfig().getStringList(
							target.getUniqueId().toString() + ".info");
					l.add("Warned by " + "(" + sender.getName() + ")" + " for "
							+ "(" + warnmessage + ")" + " in category " + "("
							+ args[1] + ")" + " at " + "(" + format.format(now)
							+ ")");
					plugin.dFile.getConfig().set(

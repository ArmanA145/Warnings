			...
			if (args.length >= 2) {
					...
					for (Player all : Bukkit.getServer().getOnlinePlayers()) {
						if (all.hasPermission("warnings.receivebroadcast")) {
							all.sendMessage(ChatColor
									...
									.replace(
											"%maxwarnings%",
											Integer.toString(plugin.getConfig()
													.getInt("maxWarnings"))));
						}
					}
				}
				int current = plugin.dFile.getConfig().getInt(
						target.getUniqueId().toString() + ".warnings", 0);
						...
						List<String> l = plugin.dFile.getConfig()
								.getStringList(
										target.getUniqueId().toString()
												+ ".info");
						l.add("Warned by " + "(" + sender.getName() + ")"
								+ " for " + "(" + warnmessage + ")" + " at "
								+ "(" + format.format(now) + ")");
						plugin.dFile.getConfig().set(
				...
				for (String msg : plugin.getConfig().getStringList(
					...
					target.sendMessage(ChatColor
							...
							.replace(
									"%maxwarnings%",
									Integer.toString(plugin.getConfig().getInt(
											"maxWarnings"))));
					if (plugin.dFile.getConfig().getInt(
							target.getUniqueId().toString() + ".warnings") > plugin
							.getConfig().getInt("maxWarnings")) {
						if (plugin.getConfig().getBoolean(
								"usePunishmentCommands")) {
							for (String pc : plugin.getConfig().getStringList(
									"punishmentCommands")) {
								Bukkit.getServer()
										.dispatchCommand(

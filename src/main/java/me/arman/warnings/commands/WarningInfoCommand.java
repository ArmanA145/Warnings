		...
		if (cmd.getName().equalsIgnoreCase("warninginfo")) {
			if (!plugin.getConfig().getBoolean("logSpecificInfo")) {
				sender.sendMessage(ChatColor.RED
						+ "Advanced information logging has been disabled!");
				return true;
			}
			if (!(sender instanceof Player)) {
				if (args.length == 0) {
					sender.sendMessage(ChatColor.RED
							+ "Usage: /warninginfo <player>");
					return true;
				}
				if (args.length >= 1) {
					OfflinePlayer target = Bukkit.getServer().getOfflinePlayer(
							args[0]);
					for (String i : plugin.getConfig().getStringList(
							"warningInfoMessage")) {
			...
			if (sender instanceof Player) {
				...
				if (!p.hasPermission("warnings.info")) {
					p.sendMessage(ChatColor.RED
							+ "You can not check your warning info!");
					return true;
				}
				if (args.length == 0) {
					for (String i : plugin.getConfig().getStringList(
					...
					if (!p.hasPermission("warnings.info.others")) {
						p.sendMessage(ChatColor.RED
								+ "You can not check other player's warning information!");
						return true;
					}
					for (String i : plugin.getConfig().getStringList(
							"warningInfoMessage")) {

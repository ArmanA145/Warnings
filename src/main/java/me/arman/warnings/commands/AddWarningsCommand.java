		...
		if (cmd.getName().equalsIgnoreCase("addwarnings")) {
			...
			if (args.length <= 1) {
				sender.sendMessage(ChatColor.RED
						+ "Usage: /addwarnings <player> <amount>");
				return true;
				...
				if (num >= plugin.getConfig().getInt("maxWarnings")
						&& !(plugin.getConfig().getInt("maxWarnings") == -1)
						|| current + num >= plugin.getConfig().getInt(
								"maxWarnings")
						&& !(plugin.getConfig().getInt("maxWarnings") == -1)) {
				...
				if (num < 0) {
					sender.sendMessage(ChatColor.RED
							+ "Use the /takewarnings command to take warnings!");
					return true;
				}
				...
				sender.sendMessage(ChatColor.GREEN + "Added " + num
						+ " warnings to " + target.getName() + "!");
